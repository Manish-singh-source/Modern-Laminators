(function ($) {
	...
	if($('.active-sticky-header').length){
		...
		$window.on("scroll", function() {
			...
			$("header.active-sticky-header .header-sticky").toggleClass("hide", false);
			$("header.active-sticky-header .header-sticky").toggleClass("active", (fromTop > headerHeight + 20));
		});
	}	
	...
	/* Skill Bar Metal */
	if ($('.skills-progress-bar-metal').length) {
		$('.skills-progress-bar-metal').waypoint(function() {
			$('.skillbar-metal').each(function() {
				$(this).find('.count-bar-metal').animate({
				width:$(this).attr('data-percent')
				},2000);
			});
		},{
	...
	/* Parallaxie js */
	var $parallaxie = $('.parallaxie').not('.hero-slider .parallaxie');
